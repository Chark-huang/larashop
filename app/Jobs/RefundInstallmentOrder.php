...
class RefundInstallmentOrder implements ShouldQueue
{
    ...
    {
        $this->order = $order;
        Log::debug('RefundInstallmentOrder Job handle order');
    }
    ...
    {
        Log::debug('RefundInstallmentOrder Job handle START');
        // 如果商品订单支付方式不是分期付款, 订单未支付, 订单退款状态不是退款中, 则不执行后面的逻辑
        Log::debug('RefundInstallmentOrder Job handle order',$this->order->toArray());
        ...
        if (!$installment = Installment::query()->where('order_id', $this->order->id)->first()){
            Log::debug('RefundInstallmentOrder Job handle installment',$installment->toArray());
            return;
        }
        Log::debug('RefundInstallmentOrder Job handle installment',$installment->toArray());
        // 遍历对应分期付款的所有还款计划
        foreach ($installment->items as $item) {
            Log::debug('RefundInstallmentOrder Job handle item',$item->toArray());
